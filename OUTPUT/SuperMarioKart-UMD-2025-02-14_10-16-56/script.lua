...
function isDone()
    ...
    --print(data.currSec)
    if (data.current_checkpoint == 4) and (data.kart1_speed > 0) then--(data.prev_checkpoint == 2) then
    ...
    elseif (data.currSec > 15) and (data.currSec < 150) then -- prevents the weird bogus 5555 I'm getting
        print('I have timed out at ', data.current_checkpoint)
        return true
    end
