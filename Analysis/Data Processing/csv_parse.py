...
x_pos = df['x_position']
y_pos = df['y_position']
checkpoints = df['current_checkpoint']
surface = df['surface']
speed = df['speed']
...
for j in range(Na):
    ...
    x_pos_series, y_pos_series, checkpoints_series, surface_series, speed_series = [], [], [], [], []
    for i in range(Nt):
        ...
        # Now do for other positions as well
        checkpoints_agents = checkpoints[i]
        checkpoints_series.append(checkpoints_agents[j])
        
        # Now do for other positions as well
        surface_agents = surface[i]
        surface_series.append(surface_agents[j])

        # Now do for other positions as well
        speed_agents = speed[i]
        speed_series.append(speed_agents[j])

    data = {} # clear from previous j
    data = {'x_position': x_pos_series,
            'y_position': y_pos_series,
            'checkpoints': checkpoints_series,
            'surfaces': surface_series,
            'speed': speed_series}
