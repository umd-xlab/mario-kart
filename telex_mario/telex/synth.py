...
def find_filenames(path, suffix=".csv"):
    if os.path.isfile(path): # If we get a file, just parse the file
        if not path.endswith(suffix):
            raise ValueError("File does not have expected suffix {}".format(suffix))
        return [path]
    elif os.path.isdir(path): # If we get a path, pick the first file in it (original code)
        filenames = os.listdir(path)
        return [os.path.join(path, filename) for filename in filenames if filename.endswith(suffix)]
    else:
        raise ValueError("Provided path is neither a file nor a directory: {}".format(path))
